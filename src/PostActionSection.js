...
PostActionSection.ProtoTypes = {
    liked: ProtoTypes.bool.isRequired,
    likes: ProtoTypes.array.isRequired,
    showComments: ProtoTypes.bool.isRequired
};

const mapStateToProps = (state,ownProps) => {
    const {postId} = ownProps;
    const {likes,showComments} = state.posts[postId];
    const comments = state.commentIds.find(commentIds => state.comments[commentIds].id ==postId);
